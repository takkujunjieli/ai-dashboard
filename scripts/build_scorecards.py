...
(每股一行,每维 = "分 (核心理由)")。4 维,单分,-5~+5(0=中性,正=好/看多,负=差/看空):
  Operation 经营   —— 财务指标(含资本支出)、股权变化(回购/稀释)
  Leadership 管理层 —— 执行/战略/可信度/Adaptability/Alignment/政府关系
  Externality 外部 —— 行业竞争、宏观压力、新闻(监管/罚款)
  Valuation 估值   —— 裁决层:price vs 上面这份质量(benchmark 未定,暂给方向占位)
理由用括号括在分数边上,只写该维得分/减分的**核心**驱动(不逐项)。草稿,非投资建议。
...
FEATURES = ["Operation", "Leadership", "Externality", "Valuation"]

# direction: L=多头 / S=空头 / W=Watch(无仓位)。每股 4 项按 FEATURES 顺序 = (score, rationale)。
# 分值 -5~+5。★=需当前数据校准,⚠=身份/数据存疑。Leadership 为新维,首版据公开认知,待校准。
STOCKS = {
 "NVDA": ("W", [
 ...
 "SNDK": ("L", [
  ...
  (-1, "P/E 20.8 但为 NAND 周期峰盈利;分拆后~40×;$1,546 已核实(mktcap $227B)")]),
 "SKHY": ("W", [
 ...
 "COHR": ("W", [
  ...
  (-2, "trailing P/E 68(GAAP)/P/B 5.1/mktcap $55B;fwd 非GAAP EPS 年化~$8→fwd P/E ~34x、增长下不算离谱 -1;52wk 低$94.55(2025-09)→高$440(2026-06)→现~$270=3x off low/-40% off high、高 beta 情绪驱动、GF Value 标 ~300% 高估 -2;计价 AI 光通信持续超增、perfection priced -1")]),
 "CCL": ("L", [
 ...
 "AMD": ("W", [
  ...
  (0, "★高倍数已计入激进 AI GPU 份额假设——做空核心")]),
 "CRWV": ("W", [
  (-1, "极重 capex(买 GPU)+巨额债务、FCF 深负;高杠杆+GPU 折旧+客户集中,脆弱"),
  (-1, "年轻团队、规模化执行未验证;重债依赖/客户集中,信誉风险"),
  (0, "neocloud 竞争+超大厂自建威胁;AI capex 顺风但融资/利率敏感极高"),
  (-2, "★高估值计入完美执行")]),
 "NBIS": ("W", [
...
print(f"生成 {len(ORDER)} 份个股 CSV + _summary.csv(4 维,单分)→ {OUT}")
